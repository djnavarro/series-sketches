...
make_sketch <- function(seed, name, version) {
  ...
  values <- tibble::tibble(
    ...
    xctr_1 = (x + 2 * xend) / 3 + pull_1 * x_focus_1,
    yctr_1 = (y + 2 * yend) / 3 + pull_1 * y_focus_1,
    xctr_2 = (x + xend) / 2 + pull_2 * x_focus_2,
    yctr_2 = (y + yend) / 2 + pull_2 * y_focus_2,
...
for(s in 901:1000) make_sketch(s, name, version)
