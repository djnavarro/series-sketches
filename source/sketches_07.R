...
make_sketch <- function(seed, name, version) {
  ...
  x_focus <- runif(1, min = -2, max = 2)
  y_focus <- runif(1, min = -2, max = 2)
  pull <- .5
  values <- tibble::tibble(
    ...
    xend = x + cos(angle) * 5,
    yend = y + sin(angle) * 5,
    xctr = (1 - pull) * (x + xend) / 2 + pull * x_focus,
    yctr = (1 - pull) * (y + yend) / 2 + pull * y_focus,
...
for(s in 701:800) make_sketch(s, name, version)
